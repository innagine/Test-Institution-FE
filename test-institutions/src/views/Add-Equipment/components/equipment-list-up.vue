<!--
 * @Description: file 检测仪器列表（已上架）
 * @Author: IMAGINE
 * @Date: 2021-04-22 11:26:13
 * @LastEditors: IMAGINE
 * @LastEditTime: 2021-05-23 18:11:03
-->
<template>
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="state" label="状态">  
          <template>
            <el-tag
              type="success"
              disable-transitions>已上架</el-tag>
          </template>
        </el-table-column>
    ...
    requestChange(sendUrl,Id,index){
      axios({
        ...
        if(res.data.ret===1){
          ...
          console.log("设备下成功",res);
        }else{
          console.log("设备下架失败",res);
          this.$notify({title: "消息",message: "操作失败",type: "warning",});
        }
      }).catch((err)=>{
        console.log("设备下架失败",err);
        this.$notify({title: "消息",message: "操作失败",type: "warning",});
      })
    },
  },
